...
- **Reads**: the flow type or lifecycle goal, the trigger event, a versioned segment definition (from the user or from [list-segment-builder](../../setup/list-segment-builder/SKILL.md) when present), current consent/suppression snapshot refs, the versioned creative/HTML bindings for each step when available, an ESP flow/automation export (own data) and current cadence/complaint signals, and one SEND profile (`promotional|retention|cold-outbound|newsletter`).
...
4. **Bind segmentation and privacy** — tie each flow to `segment_ref`, `definition_version`, `definition_hash`, and evaluation time from [list-segment-builder](../../setup/list-segment-builder/SKILL.md), plus current consent/suppression snapshot refs. Member joins use opaque `subject_ref`; raw addresses remain transient at the ESP boundary. Do not enroll suppressed/non-consented subjects.
...
- [Email Send Control](references/send-control.md) — opaque subject refs, immutable segment/payload bindings, create-vs-send separation, and receipt/partial semantics.
...
- **If the flow map is ready for the gate**: [email-quality-auditor](../../deliver/email-quality-auditor/SKILL.md) — score the profile-weighted EQS and enforce N1 (unsubscribe integrity) and the other vetoes.
