...
Score only the placement-relevant `S` sub-items from [send-benchmark.md](../../../../references/send-benchmark.md): inbox-placement against the declared provider/seed threshold and normalized spam-complaint rate. Name the typed profile and its `S` weight (0.30 promotional / 0.20 retention / 0.35 cold-outbound / 0.25 newsletter). Do **not** score auth (`S1`), the static reputation setup, or the full `S` dimension roll-up — hand the placement snapshot to [email-quality-auditor](../../email-quality-auditor/SKILL.md) to fold into the EQS.
